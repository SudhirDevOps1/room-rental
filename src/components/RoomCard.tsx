...
export const RoomCard: React.FC<RoomCardProps> = ({ room, userCoords, onRequestRent, viewMode = 'grid' }) => {
  ...
  if (viewMode === 'list') {
    return (
      <div 
        ...
        className="bg-white rounded-3xl border border-slate-200/80 hover:border-indigo-500/50 shadow-xs hover:shadow-xl transition-all duration-300 overflow-hidden flex flex-col sm:flex-row group cursor-pointer"
      >
        {/* Left Image Box */}
        <div className="sm:w-72 h-56 sm:h-auto relative flex-shrink-0 bg-slate-100 overflow-hidden">
          <img
        ...
        <div className="p-6 flex-1 flex flex-col justify-between">
          <div>
            <div className="flex items-center justify-between gap-2 flex-wrap">
              <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-md bg-amber-100 text-amber-900 font-extrabold text-xs">
                Ward No: {room.wardNo}
              </span>
            ...
            <h3 className="text-lg font-bold text-slate-900 group-hover:text-indigo-600 transition duration-300 mt-2 line-clamp-1">
              {room.title}
            </h3>
            ...
            <div className="flex flex-wrap gap-1.5 mt-3">
              {room.amenities?.slice(0, 5).map(am => (
                <span key={am} className="px-2 py-1 bg-slate-100 rounded-lg text-[11px] font-medium text-slate-700 border border-slate-200/60">
                  {am}
                </span>
              ))}
              {room.amenities?.length > 5 && (
                <span className="px-2 py-1 bg-indigo-50 rounded-lg text-[11px] font-bold text-indigo-600">
          ...
          <div className="pt-4 mt-4 border-t border-slate-100 flex items-center justify-between gap-3">
            <div>
              ...
              <div className="text-2xl font-extrabold text-indigo-950 flex items-baseline gap-0.5">
                <IndianRupee className="w-5 h-5 text-indigo-600" />
                <span>{room.rent.toLocaleString('en-IN')}</span>
                <span className="text-xs font-normal text-slate-500">/{room.rentType}</span>
              </div>
            </div>
  ...
  return (
    <div 
      onClick={() => navigate(`/room/${room.id}`)}
      className="bg-white rounded-3xl border border-slate-200/80 hover:border-indigo-500/50 shadow-xs hover:shadow-xl transition-all duration-300 overflow-hidden flex flex-col group cursor-pointer relative"
    >
      {/* Top Image gallery */}
      ...
      <div className="p-5 flex-1 flex flex-col justify-between">
        <div>
          <div className="flex items-center justify-between text-xs mb-1.5">
            <span className="font-extrabold text-amber-900 bg-amber-100 px-2.5 py-0.5 rounded-md">
              Ward No: {room.wardNo}
            </span>
          ...
          <h3 className="text-base font-bold text-slate-900 group-hover:text-indigo-600 transition duration-300 line-clamp-1">
            {room.title}
          </h3>
          ...
          <div className="flex flex-wrap gap-1 mt-3">
            {room.amenities?.slice(0, 4).map(am => (
              <span key={am} className="px-2 py-1 bg-slate-50 rounded-lg text-[10px] font-semibold text-slate-600 border border-slate-200/70">
                {am}
              </span>
            ))}
            {room.amenities?.length > 4 && (
              <span className="px-2 py-1 bg-indigo-50 rounded-lg text-[10px] font-bold text-indigo-600">
                +{room.amenities.length - 4}
              </span>
            )}
          </div>
        ...
        <div className="pt-4 mt-4 border-t border-slate-100 flex items-center justify-between gap-2">
          <div>
            <div className="text-[10px] text-slate-400 uppercase font-bold">Monthly Rent</div>
            <div className="text-xl font-extrabold text-indigo-950 flex items-baseline gap-0.5">
              <IndianRupee className="w-4 h-4 text-indigo-600" />
              <span>{room.rent.toLocaleString('en-IN')}</span>
              <span className="text-[11px] font-normal text-slate-500">/{room.rentType}</span>
            </div>
          </div>
          ...
          {room.available ? (
            ...
            <span className="px-3 py-1.5 rounded-xl bg-slate-100 text-slate-500 font-bold text-xs flex items-center gap-1">
