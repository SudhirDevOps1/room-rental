...
export const Home: React.FC = () => {
  ...
  return (
    <div className="space-y-24 pb-12 animate-fadeIn">
      ...
      <section className="relative pt-12 pb-20 px-4 sm:px-6 lg:px-8 bg-gradient-to-b from-indigo-900 via-indigo-800 to-purple-900 rounded-3xl mx-2 sm:mx-6 lg:mx-8 mt-4 overflow-hidden shadow-2xl text-white">
        ...
        <div className="max-w-4xl mx-auto text-center space-y-6 relative z-10">
          ...
          {/* Search Card Wizard */}
          <div className="bg-white p-3 sm:p-4 rounded-3xl shadow-2xl max-w-3xl mx-auto text-slate-800 border border-slate-100 transform hover:scale-[1.01] transition duration-300">
            <form onSubmit={handleSearchSubmit} className="flex flex-col sm:flex-row items-center gap-3">
              
              {/* City selector */}
              <div className="flex-1 w-full bg-slate-50 hover:bg-slate-100 p-3 rounded-2xl border border-slate-200/80 transition flex items-center gap-2.5">
                <MapPin className="w-5 h-5 text-indigo-600 flex-shrink-0" />
                <div className="flex flex-col text-left w-full">
                  <span className="text-[10px] font-bold uppercase text-slate-400">Select Tech/Education Hub</span>
                  <select
                    value={searchCity}
              ...
              <div className="flex-1 w-full bg-slate-50 hover:bg-slate-100 p-3 rounded-2xl border border-slate-200/80 transition flex items-center gap-2.5">
                ...
                <div className="flex flex-col text-left w-full">
                  <span className="text-[10px] font-bold uppercase text-slate-400">Ward No. or Landmark</span>
                  <input
                    type="text"
                    placeholder="e.g. Ward 12, Hudson Lane"
                    value={searchWard}
                    onChange={(e) => setSearchWard(e.target.value)}
                    className="w-full bg-transparent font-bold text-sm text-indigo-950 focus:outline-none placeholder:text-slate-400"
                  />
                </div>
              </div>
            ...
            <div className="mt-3 pt-3 border-t border-slate-100 flex items-center justify-between text-xs font-semibold px-2 flex-wrap gap-2">
              ...
              <button
                ...
                className="px-3.5 py-1.5 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 rounded-xl font-extrabold flex items-center gap-1.5 transition active:scale-95 cursor-pointer"
              >
                <Navigation className={`w-3.5 h-3.5 text-indigo-600 ${isDetecting ? 'animate-spin' : 'animate-pulse'}`} />
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
        <div className="flex flex-col sm:flex-row items-start sm:items-end justify-between gap-4">
          <div>
            <span className="text-xs font-black text-indigo-600 tracking-widest uppercase">Location Priority</span>
            <h2 className="text-2xl sm:text-3xl font-black text-indigo-950 tracking-tight mt-1">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {POPULAR_CITIES.map((city) => (
            <div
              ...
              className="group relative h-64 rounded-3xl overflow-hidden shadow-md hover:shadow-2xl transition duration-500 cursor-pointer border border-slate-200"
            >
              <img
                src={city.image}
                alt={city.name}
      ...
      <section className="bg-slate-100/80 rounded-3xl p-8 sm:p-12 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 border border-slate-200/80">
        <div className="text-center max-w-3xl mx-auto space-y-3 mb-16">
          <span className="text-xs font-extrabold text-amber-600 uppercase tracking-widest bg-amber-100 px-3 py-1 rounded-full">
            Dual Persona Modes
          </span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
          ...
          {/* Seekers column */}
          <div className="bg-white rounded-3xl p-8 shadow-lg border border-indigo-100 relative overflow-hidden space-y-6">
            ...
            <div className="space-y-4 text-xs sm:text-sm text-slate-700">
              <div className="flex items-start gap-3">
                <span className="w-6 h-6 rounded-full bg-indigo-100 text-indigo-700 font-extrabold flex items-center justify-center flex-shrink-0 mt-0.5">1</span>
                <div>
                  <strong className="text-slate-900 font-extrabold">Advanced Proximity Search:</strong> Enter your ward number or use "Find Near Me" to instantly compute exact crow-fly distances in kilometers.
                </div>
              </div>

              <div className="flex items-start gap-3">
                <span className="w-6 h-6 rounded-full bg-indigo-100 text-indigo-700 font-extrabold flex items-center justify-center flex-shrink-0 mt-0.5">2</span>
                <div>
                  <strong className="text-slate-900 font-extrabold">Send Verification Request:</strong> Click "Request Room" to notify the verified owner about your intended move-in date.
                </div>
              </div>

              <div className="flex items-start gap-3">
                <span className="w-6 h-6 rounded-full bg-indigo-100 text-indigo-700 font-extrabold flex items-center justify-center flex-shrink-0 mt-0.5">3</span>
                <div>
                  <strong className="text-slate-900 font-extrabold">Instant Contact Unlock:</strong> Once the owner accepts your application, their direct mobile number gets unlocked and real-time live chat begins!
                </div>
              </div>
            </div>
          ...
          {/* Owners column */}
          <div className="bg-white rounded-3xl p-8 shadow-lg border border-amber-100 relative overflow-hidden space-y-6">
            ...
            <div className="w-14 h-14 rounded-2xl bg-amber-50 text-amber-600 flex items-center justify-center font-black text-xl shadow-inner">
              🏠
            </div>
            ...
            <div className="space-y-4 text-xs sm:text-sm text-slate-700">
              <div className="flex items-start gap-3">
                <span className="w-6 h-6 rounded-full bg-amber-100 text-amber-800 font-extrabold flex items-center justify-center flex-shrink-0 mt-0.5">1</span>
                <div>
                  <strong className="text-slate-900 font-extrabold">Register Location Flawlessly:</strong> Let our browser map pin pick up your exact geocoordinates or enter your ward number and landmark.
                </div>
              </div>

              <div className="flex items-start gap-3">
                <span className="w-6 h-6 rounded-full bg-amber-100 text-amber-800 font-extrabold flex items-center justify-center flex-shrink-0 mt-0.5">2</span>
                <div>
                  <strong className="text-slate-900 font-extrabold">Manage Live Status:</strong> When your room gets booked, toggle your room availability instantly so you don't receive spam requests.
                </div>
              </div>

              <div className="flex items-start gap-3">
                <span className="w-6 h-6 rounded-full bg-amber-100 text-amber-800 font-extrabold flex items-center justify-center flex-shrink-0 mt-0.5">3</span>
                <div>
                  <strong className="text-slate-900 font-extrabold">Screen Renter Applicants:</strong> Review tenant profiles and messages in your "Requests Received" portal before accepting.
                </div>
              </div>
            </div>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
        <div className="text-center max-w-2xl mx-auto space-y-2">
          <span className="text-xs font-black text-emerald-600 uppercase tracking-widest">Everything You Need</span>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-8 gap-4">
          {ALL_AMENITIES_LIST.map((am) => (
            <div
              ...
              className="bg-white hover:bg-indigo-50 p-4 rounded-2xl border border-slate-200/80 hover:border-indigo-300 text-center space-y-2 cursor-pointer transition shadow-2xs hover:shadow-md group"
            >
              <div className="text-2xl group-hover:scale-125 transition transform duration-300">{am.icon}</div>
              <div className="text-[11px] font-bold text-slate-800 leading-tight">{am.label}</div>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="bg-gradient-to-r from-emerald-600 to-teal-700 rounded-3xl p-8 sm:p-12 max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 shadow-2xl text-white flex flex-col md:flex-row items-center justify-between gap-8">
        ...
        <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
          ...
          <button
            ...
            className="px-6 py-4 rounded-2xl bg-emerald-900 hover:bg-emerald-800 text-white font-black text-xs sm:text-sm shadow-xl transition transform hover:scale-105 active:scale-95 flex items-center justify-center gap-2 border border-emerald-700 cursor-pointer"
          >
